...
class FastBleScanHandler(
    ...
    override fun onListen(arguments: Any?, events: EventChannel.EventSink?) {
        eventSink = events
        targetAddress = arguments as? String

        if (targetAddress == null) {
            events?.error("INVALID_ARG", "Device address is required", null)
            return
        }
    ...
    private fun startLowLatencyScan() {
        ...
        try {
            ...
            Log.d(TAG, "Low-latency scan started for $targetAddress")
    ...
    private fun parseWeightFromManufacturerData(record: android.bluetooth.le.ScanRecord): Int? {
        ...
        while (index < records.size - 1) {
            ...
            if (type == 0xFF && length >= WEIGHT_OFFSET + WEIGHT_LENGTH + 2) {
                val cmdBytes = records.copyOfRange(index + 2, index + length + 1)
                val result = parseWeight(cmdBytes, WEIGHT_OFFSET)
                if (result != null) return result

                val adjustedOffset = WEIGHT_OFFSET - 2
                val payloadBytes = records.copyOfRange(index + 4, index + length + 1)
                val result2 = parseWeight(payloadBytes, adjustedOffset)
